...
if(!Number.isInteger(body.daily_limit)||body.daily_limit<1||body.daily_limit>50)throw Error("Daily cap must be 1–50");if(typeof body.sender_name!=="string"||!body.sender_name.trim()||/[\r\n]/.test(body.sender_name))throw Error("Sender name required");
await result(db().from("settings").update({templates:ts,templates_approved:true,sending_enabled:false,daily_limit:body.daily_limit,sender_name:body.sender_name.slice(0,100),postal_address:String(body.postal_address||"").slice(0,400)}).eq("id",1));return json({ok:true,message:"Templates saved and approved. Sending paused; resume after review."});}
